...
const GuardiasPage = () => {
  ...
  const fetchGuardsForWeek = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/guards?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}`
      );
      type APIGuard = {
        id: number;
        assignedDate: string;
      ...
      const { guards: existingGuards } = (await response.json()) as {
        guards: APIGuard[];
      };
      ...
      const newAssignments: DailyAssignments[] = Array(7)
        .fill(null)
        .map((_, i) => {
      ...
      const newAssignments = new Array(7).fill(null).map((_, i) => {
        const date = new Date(startDate);
        date.setDate(startDate.getDate() + i);
        return { date, guards: [] };
      });
  ...
  return (
    <div className="w-full px-4 md:px-8 lg:px-16 py-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center w-[400px] gap-2 p-2 bg-white rounded-lg shadow-sm">
            <button
              ...
              className="p-2 rounded-md hover:bg-gray-100 text-gray-600 hover:rounded-full hover:cursor-pointer text-blue-800 font-semibold"
            >
              &lt;
            </button>
            <div className="text-center font-mono font-semibold text-blue-800 w-[180px]">
              {getWeekDisplay()}
            </div>
            <button
              ...
              className="p-2 rounded-md hover:bg-gray-100 text-gray-600 hover:cursor-pointer hover:rounded-full text-blue-800 font-semibold"
            >
              &gt;
            </button>
            <button
        ...
        <div className="space-y-6">
          {assignments.map((dailyAssignment, index) => (
            <div
              key={index}
              className="p-6 border rounded-lg bg-white shadow-md transition-shadow hover:shadow-lg"
            >
              <div className="flex justify-between items-center mb-4">
                <div>
                  <h3 className="font-bold text-xl text-gray-800 capitalize">
                    {new Date(dailyAssignment.date).toLocaleDateString(
                      "es-ES",
                      { weekday: "long" }
                    )}
                  </h3>
              ...
              <div className="space-y-4">
                {dailyAssignment.guards.length > 0 ? (
                  dailyAssignment.guards.map((guard) => (
                    <div
                      key={guard.id}
                      className="w-full grid grid-cols-1 md:grid-cols-5 items-center gap-4 bg-gray-50 p-3 rounded-lg"
                    >
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {guard.staff.rol.name}
                        </p>
                      </div>
                      <div>
                        <div className="flex items-center gap-2 text-sm text-gray-500">
                          <IdentificationIcon className="w-4 h-4" />
                          <span>{guard.staff.dni}</span>
                        </div>
                        {guard.staff.phone && (
                          <div className="flex items-center gap-2 text-sm text-gray-500">
                            <DevicePhoneMobileIcon className="w-4 h-4" />
                            <span>{guard.staff.phone}</span>
                          </div>
                        )}
                      </div>
                      <div className="flex items-center gap-2 text-gray-700">
                        <MapPinIcon className="w-5 h-5" />
                        <p className="font-medium">{guard.location.name}</p>
                      </div>
                      <div className="flex justify-center md:justify-end items-center gap-2">
                        ...
                        <button
                          onClick={() => handleDeleteAssignment(guard.id)}
      ...
      <Tooltip id="my-tooltip" place="bottom" variant="info" />
      <Tooltip id="guard-button" place="bottom" variant="info" />
      <Tooltip id="last-week-tooltip" place="bottom" variant="info" />
      <Tooltip id="next-week-tooltip" place="bottom" variant="info" />
